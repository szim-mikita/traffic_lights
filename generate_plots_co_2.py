### Needs to be joined generate_plots_mean_speed.py 
...
def main_without_ai(num_episodes=10, traffic_scale=1.0):
    ...
    for episode in range(num_episodes):
        ...
        sumo_binary = "/var/lib/flatpak/app/org.eclipse.sumo/x86_64/stable/c02efd9cf5fa44c5b8e9c6963bfc8b81ded25157f34fab931a1e8bd4fd900211/files/bin/sumo"
...
if __name__ == "__main__":
    ...
    for scale in traffic_scales:
        ...
        ai_pp = main_with_ai(num_episodes=1, traffic_scale=scale, onnx_path='models/model_phaseplan_speed_1.0.onnx', use_phaseplan=True)
        ai_no_pp = main_with_ai(num_episodes=1, traffic_scale=scale, onnx_path='models/model_no_phaseplan_speed_1.0.onnx', use_phaseplan=False)
    ...
    ax.set_ylabel("CO₂")
    ax.set_title("CO₂: Different traffic scales")
    ax.set_xticks(x)
    ax.set_xticklabels([f"Traffic {s}" for s in traffic_scales])
    ax.legend()
    plt.tight_layout()
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.savefig("plots/co2_three_1.0.png")
    plt.show()